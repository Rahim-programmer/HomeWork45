...
public class ProgramUtility {
    public static Map<String, String> parseUrl(String rawLines, String delimiter) {
        String[] pairs = rawLines.split(delimiter);
        Stream<Map.Entry<String, String>> stream = Arrays.stream(pairs)
                .map(ProgramUtility::description)
                .filter(Optional::isPresent)
                .map(Optional::get);
        return stream.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
    }

    static Optional<Map.Entry<String, String>> description(String desctiption) {
        if (!desctiption.contains("=")) {
            return Optional.empty();
        }
        String[] pair = desctiption.split("=");
        if (pair.length != 2) {
            return Optional.empty();
        }
